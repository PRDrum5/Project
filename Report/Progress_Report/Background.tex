...
The concept behind generative adversarial networks is two have two machine learning models; a generator and a discriminator.
...
\begin{equation} \label{cgans}
    \mathcal{L}_{adv} = \mathbb{E}_{x \sim X_{real}}[\log(D_{src}(x)] 
    + \mathbb{E}_{x \sim X_{real},c \sim C}[\log(1-D_{src}(G(x,c))]
\end{equation}
